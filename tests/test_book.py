...
def test_current_temperature(connected_openweather_book):
    temperature = connected_openweather_book.current_temperature(NounPhrase("New York", []))
...
def test_current_temperature_with_units(connected_openweather_book):
    ...
    assert math.isclose(temperature_celsius, convert_to_celsius(temperature_fahrenheit), abs_tol=0.4)


@pytest.mark.vcr
def test_get_current_temperature_error_response(connected_openweather_book):
    with pytest.raises(HTTPError):
        connected_openweather_book.current_temperature(NounPhrase("NonExistentCity", []))
